...
void *mymalloc(size_t size, char *file, int line)
{
    ...
    printf("Not enough memory to allocate for your object in file %s line %d", file, line);
    return NULL;
}
...
void myfree(void *ptr, char *file, int line)
{
    ...
    do
    {
        ...
        if (ptr == h->ptr)
        {
            ...
            {
                printf("Given pointer in file %s line %d was already freed.\n", file, line);
                return;
            }
            break;
        }
    ...
    if (isPtrMalloced == 0)
    {
        ...
        {
            printf("given address in file %s is not at the start of a chunk of allocated memory (line %d)\n", file, line);
        }
        else
        {
            printf("Declaration in file %s line %d attempts bad free.\n", file, line);
        }
        return;
    }
